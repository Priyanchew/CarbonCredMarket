...
async def verify_api_key(x_api_key: str = Header(..., alias="X-API-Key", description="API key for authentication")):
    ...
    """
    try:
        logger.info(f"Received API key: {x_api_key[:8]}...")
        ...
        logger.info(f"Database query result: {len(response.data) if response.data else 0} rows found")
        ...
        logger.info(f"API key verified for user: {user['email']}")
